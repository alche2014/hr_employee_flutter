...
class AddTrainings extends StatelessWidget {
  final data, uid, editable;
  const AddTrainings({Key? key, this.data, this.uid, this.editable})
      : super(key: key);

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: buildMyAppBar(context, 'Trainings', true),
      body: Stack(
        children: [
          Container(
            margin:
                const EdgeInsets.only(left: 15, right: 12, bottom: 15, top: 15),
  ...
  validateAndUpdate() async {
    ...
    if (form!.validate()) {
      ...
      DocumentReference trainings = guest == 0
          ...
          : FirebaseFirestore.instance.collection("employees").doc(userId);
      trainings.update({
        "trainings": FieldValue.arrayRemove([widget.userData])
      });
      Map<String, dynamic> serializedMessage = {
        "name": nameController.text.isEmpty
            ? ""
